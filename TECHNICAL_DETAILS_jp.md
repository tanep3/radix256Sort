...
Pythonの `list` は Rust `Vec` へ効率的に変換され、Rust側でネイティブ処理されます。NumPy 配列については、（必要であれば）生メモリバッファを直接扱う設計も可能であり、本実装はその高速化構造を示す教育的な題材となっています。
...
3.  **完了**: 最終パス終了後、データが `from` と `to` のどちらにあるかは実装上のスワップ順序に依存します。本実装では、どちらに格納されていても正しく結果を返すように設計されています。
